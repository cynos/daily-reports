...
func Injections(first, last string) {
	...
	for i := 0; i < days; i++ {
		...
		for k, v := range date {
			res = append(res, idxInjections(k, v, dbs))
		}

		fmt.Println("+", res)
		pushInjections(date["lowdate"], res, dbl)
	...
	fmt.Println("Done, index injections")
	fmt.Println("================================================")
}

func idxInjections(keyDate, valDate string, db *sql.DB) []string {
	dobreak := false
	index := 10000000
	plus := 1000000
	minus := 500000
	...
	for dobreak == false {
		...
		query := `select id, (created_at at time zone 'Asia/Karachi') from mobilink_injections where id < ` + strconv.Itoa(index) + " order by id desc limit 100;"
...
func pushInjections(date string, data [][]string, db *sql.DB) {
	query := `insert into injections_index (date, idx_min, idx_max) values ($1, $2, $3)`
	_, err := db.Exec(query, date, data[0][0], data[1][0])
	checkError("failed to insert injections_index - ", err)
}
